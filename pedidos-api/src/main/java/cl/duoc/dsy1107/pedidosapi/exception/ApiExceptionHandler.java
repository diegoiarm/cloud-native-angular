...
// Errores de negocio en JSON con el código HTTP adecuado.
// 401/403 los responde Spring Security antes de llegar aquí.
@RestControllerAdvice
public class ApiExceptionHandler {

    @ExceptionHandler({PedidoNoEncontradoException.class, ProductoNoEncontradoException.class})
    ResponseEntity<Map<String, Object>> noEncontrado(RuntimeException e) {
        return error(HttpStatus.NOT_FOUND, e.getMessage());
    }

    @ExceptionHandler({TransicionInvalidaException.class, StockInsuficienteException.class})
    ResponseEntity<Map<String, Object>> conflicto(RuntimeException e) {
        return error(HttpStatus.CONFLICT, e.getMessage());
    }

    @ExceptionHandler(CatalogoNoDisponibleException.class)
    ResponseEntity<Map<String, Object>> catalogoNoDisponible(CatalogoNoDisponibleException e) {
    ...
    ResponseEntity<Map<String, Object>> ilegible(HttpMessageNotReadableException e) {
        return error(HttpStatus.BAD_REQUEST, "JSON inválido o estado desconocido");
    }
